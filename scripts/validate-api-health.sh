...
test_api_health() {
    ...
    RESPONSE_TIME=$(curl -s -o /dev/null -w "%{time_total}" "$url/health" --max-time 10)
    ...
    RESPONSE=$(curl -s "$url/health" --max-time 10)
    ...
    CORS_ORIGIN=$(curl -s -I -H "Origin: https://mango-sky-0ba265c0f.1.azurestaticapps.net" "$url/health" | grep -i "access-control-allow-origin" || echo "")
    ...
    OPTIONS_CODE=$(curl -s -o /dev/null -w "%{http_code}" -X OPTIONS "$url/health" --max-time 10)
...
# Test all APIs
test_api_health "$PUMP_API_URL" "Pump Report API"
test_api_health "$AUTH_API_URL" "Medical Auth API"
test_api_health "$SCHEDULE_API_URL" "Schedule API"
