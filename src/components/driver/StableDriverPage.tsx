...
export default function StableDriverPage() {
  ...
  const generateSOAP = async () => {
    ...
    try {
      ...
      logInfo('StableDriverPage', 'Processing transcript with AI service', {
      ...
      logInfo('StableDriverPage', 'Successfully generated SOAP note', {
        templateUsed: selectedTemplate?.name
      });
    } catch (error: any) {
      logError('StableDriverPage', 'Failed to generate SOAP note', {
        error: error.message,
        patientId
      });
      ...
      if (transcript && transcript.trim().length > 0) {
        ...
        setError('');
        logDebug('StableDriverPage', 'Using fallback note generation', {});
      }
    } finally {
      setLoading(false);
    }
