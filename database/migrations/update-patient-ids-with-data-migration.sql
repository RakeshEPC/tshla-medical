-- Safe Migration: Patient ID Redesign with Data Migration
-- Handles existing views, dependencies, and existing patient data
-- Date: 2026-01-05
...
-- Step 4: Set patient_id to NULL temporarily (we'll generate new random IDs)
UPDATE unified_patients SET patient_id = NULL;

-- Step 5: Now we can safely change the column type
ALTER TABLE unified_patients
ALTER COLUMN patient_id TYPE VARCHAR(8);

-- Step 6: Create or modify tshla_id column
DO $$
BEGIN
  IF NOT EXISTS (
...
-- Step 7: Drop old validation triggers if they exist
...
-- Step 8: Create new validation function for TSH ID format
...
-- Step 9: Create trigger for TSH ID validation
CREATE TRIGGER validate_tsh_id_trigger
BEFORE INSERT OR UPDATE ON unified_patients
FOR EACH ROW
EXECUTE FUNCTION validate_tsh_id();

-- Step 10: Create new validation function for Patient ID format
CREATE OR REPLACE FUNCTION validate_patient_id()
RETURNS TRIGGER AS $$
BEGIN
  -- Patient ID must be exactly 8 digits (but can be NULL during migration)
...
-- Step 11: Create trigger for Patient ID validation
CREATE TRIGGER validate_patient_id_trigger
BEFORE INSERT OR UPDATE ON unified_patients
FOR EACH ROW
EXECUTE FUNCTION validate_patient_id();

-- Step 12: Add indexes for improved search performance
...
-- Step 13: Recreate the v_patients_incomplete_data view with new schema
...
COMMENT ON COLUMN unified_patients.old_patient_id IS 'Backup of old patient_id format for reference. Not used in new system.';
...
-- Migration Notes:
-- 1. Old patient_id values are preserved in old_patient_id column
-- 2. New patient_id values (8-digit random) will be generated by the import script
-- 3. Existing patients will have patient_id = NULL until they are accessed/updated
-- 4. The import script will generate new IDs for all imported patients
