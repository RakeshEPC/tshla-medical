import{o as n}from"./openai.service-BgTyMgqK.js";import"./pumpDataComplete-A26PerGF.js";import{g as p,b as l}from"./index-JAW6HvUl.js";class c{async generateRecommendation(e){try{if(p("pumpDrivePureAI","Starting pure AI analysis",{}),!e.clarifyingResponses){const o=await this.determineIfClarificationNeeded(e);if(o.questions.length>0)return{topChoice:{name:"",score:0,reasons:[]},alternatives:[],keyFactors:[],personalizedInsights:"",clarifyingQuestions:o.questions,needsClarification:!0}}return await this.generateFinalRecommendation(e)}catch(t){throw l("pumpDrivePureAI","Error in pure AI analysis",{error:t}),new Error("Unable to process recommendation at this time. Please try again.")}}async determineIfClarificationNeeded(e){const t=this.buildInitialAnalysisPrompt(e),o=await n.processText(t,{model:"gpt-4",temperature:.7,maxTokens:2e3});try{const i=JSON.parse(o);return{questions:i.clarifyingQuestions||[],topCandidates:i.topCandidates||[]}}catch{return{questions:[],topCandidates:[]}}}async generateFinalRecommendation(e){const t=this.buildFinalRecommendationPrompt(e),o=await n.processText(t,{model:"gpt-4",temperature:.7,maxTokens:2e3});try{const i=JSON.parse(o);return{topChoice:i.topChoice,alternatives:i.alternatives||[],keyFactors:i.keyFactors||[],personalizedInsights:i.personalizedInsights||"",needsClarification:!1}}catch(i){throw l("pumpDrivePureAI","Failed to parse final recommendation",{error:i}),new Error("Unable to generate recommendation")}}buildInitialAnalysisPrompt(e){return`You are an expert diabetes educator. Analyze this patient's pump preferences and determine if you need clarifying questions to make a confident recommendation.

PATIENT INPUT DATA:
${this.formatUserInputs(e)}
...
}`}buildFinalRecommendationPrompt(e){return`You are an expert diabetes educator. Provide your final pump recommendation based on all available information.
...
${e.clarifyingResponses?`
CLARIFYING RESPONSES:
${this.formatClarifyingResponses(e.clarifyingResponses)}
...
}`}formatUserInputs(e){let t="";return e.sliders&&(t+=`LIFESTYLE SLIDERS:
`,Object.entries(e.sliders).forEach(([o,i])=>{t+=`- ${o}: ${i}/10
`}),t+=`
`),e.selectedFeatures&&e.selectedFeatures.length>0&&(t+=`SELECTED FEATURES (pump-specific preferences):
`,e.selectedFeatures.forEach(o=>{t+=`- ${o}
`}),t+=`
`),e.userText&&(t+=`USER'S STORY AND PRIORITIES:
`,t+=`"${e.userText}"

`),t}formatClarifyingResponses(e){let t="";return Object.entries(e).forEach(([o,i])=>{t+=`Q: ${o}
A: ${i}

`}),t}formatPumpFeatureMappings(){return`When user selects specific features, they directly map to these pumps:
...
CRITICAL: If user selected ANY of these features, that pump should be heavily favored in your recommendation.`}formatSliderMeanings(){return`Slider values (1-10 scale) indicate user preferences:
...
→ Low time dedication favors: Beta Bionics iLet (automated), Omnipod 5 (disposable pods)`}formatPumpDatabase(){return`COMPLETE INSULIN PUMP COMPARISON DATA (23 Dimensions):
...
- Twiist: Weighs only 2 ounces, Apple Watch bolusing, emoji-based dosing interface`}extractWeight(e){if(e.dimensions.tubing.type==="tubeless")return"Tubeless pod";const t=e.pros?.find(o=>o.toLowerCase().includes("oz")||o.toLowerCase().includes("ounce")||o.toLowerCase().includes("weight"));return t||e.dimensions.discretion?.size||"Standard size"}saveRecommendation(e){sessionStorage.setItem("pumpdrive_recommendation",JSON.stringify(e)),sessionStorage.setItem("pumpdrive_recommendation_timestamp",Date.now().toString()),sessionStorage.setItem("pumpdrive_recommendation_source","pure_ai")}collectUserInputData(){const e={},t=sessionStorage.getItem("pumpDriveSliders");t&&(e.sliders=JSON.parse(t));const o=sessionStorage.getItem("pumpDriveSelectedFeatures");if(o){const s=JSON.parse(o);e.selectedFeatures=s.map(a=>a.name||a.id||a)}const i=sessionStorage.getItem("pumpDriveFreeText");if(i){const s=JSON.parse(i);e.userText=s.currentSituation||s.userStory||""}const r=sessionStorage.getItem("pumpDriveClarifyingResponses");return r&&(e.clarifyingResponses=JSON.parse(r)),e}}const g=new c;export{g as pumpDrivePureAI};
//# sourceMappingURL=pumpDrivePureAI.service-CnfqUyZ1.js.map
